    ...
    if(sizeof($result) > 0){
      foreach ($result as $key) {
          $item = array(
            ...
            'delivered_date_with_time'=> $key['date_delivered'] ? Carbon::createFromFormat('Y-m-d H:i:s', $key['date_delivered'])->copy()->tz($this->response['timezone'])->format('F j, Y H:i A') : null,
