  ...
  public function manageResults($result){
    ...
    foreach ($result as $key) {
      $item = array(
        ...
        'date_of_delivery_formatted'  => Carbon::createFromFormat('Y-m-d H:i:s', $key['date_of_delivery'])->copy()->tz($this->response['timezone'])->format('m/j/Y'),
        'merchant_to' => app($this->merchantClass)->getColumnByParams('id', $key['merchant_to'], ['id', 'name', 'address']),
        'status'        => $key['status'],
    ...
    if(sizeof($result) > 0){
      foreach ($result as $key) {
          $item = array(
            ...
            'date_of_delivery'  => Carbon::parse($key['date_of_delivery'])->format('d/m/Y H:i'),
            ...
            'delivered_date_with_time'=> $key['date_delivered'] ? Carbon::createFromFormat('Y-m-d H:i:s', $key['date_delivered'])->copy()->tz($this->response['timezone'])->format('d/m/Y H:i') : null,
