...
class DailyLoadingListController extends APIController
{
  ...
  public function createMultiple(Request $request)
  {
    ...
    if ($data['orders'] && sizeof($data['orders']) > 0) {
      for ($index = 0; $index < sizeof($data['orders']); $index++) {
        $item = $data['orders'][$index];

        if ($this->checkIfExist('order_request_id', $item['id']) == true) {
        } else {
          $nData = array(
