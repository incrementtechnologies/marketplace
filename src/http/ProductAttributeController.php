...
class ProductAttributeController extends APIController
{
    ...
    public function getAttributeByProductId($productId){
      $result = ProductAttribute::where('product_id', '=', $productId)->where('deleted_at', '=', null)->get();
      return (sizeof($result) > 0) ? $result : null;
    }
    ...
    public function getByParamsWithMerchant($column, $value, $merchantId){
      ...
      if(sizeof($result) > 0){
        ...
        foreach ($result as $key) {
          ...
          $transferredProduct = app('Increment\Marketplace\Http\TransferredProductController')->getTotalTransferredByMerchant($key['id'], $merchantId);
