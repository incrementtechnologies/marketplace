	...
	public function createDeliveryTransfer(Request $request)
	{
		...
		if ($this->response['data'] > 0) {
			...
			foreach ($products as $key) {
				...
				// trace us trace is broken on bundled products
				$traceIsBreak = app($this->bundledProductController)->getBrokenTrace($key['product_trace']);
				if ($traceIsBreak !== null) {
					// Exist product trace on bundled products

					// Check if product already transferred to other merchants
					...
					// if transferred, update the trasferred product to inactive
					if ($existTrace !== null) {
						// make it inactive
					...
					// product not exist on transferred product
				...
				// check if product is a bundled trace
				$existInBundled = [];
