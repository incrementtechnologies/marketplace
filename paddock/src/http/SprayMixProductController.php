...
class SprayMixProductController extends APIController
{
  ...
  public function retrieveByParams(Request $request){
    ...
    if(isset($data['condition']) && $data['condition'][0]['column'] == 'spray_mix_id'){
      $this->response['spray_mix'] = app($this->sprayMixClass)->getByParams('id', $data['condition'][0]['value'], ['id', 'merchant_id', 'name', 'short_description', 'application_rate', 'minimum_rate', 'maximum_rate', 'status', 'created_at']);
    }
    return $this->response();
  }
}
