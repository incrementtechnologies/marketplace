    ...
    if (sizeof($task) > 0) {
      ...
      for ($i = 0; $i <= sizeof($this->response['data']) - 1; $i++) {
        ...
        if ($paddockPlan !== null) {
          ...
          $crop = Crop::where("id", "=", $paddockPlan['crop_id'])->get();
          ...
          $this->response['data'][$i]['start_date'] = $temp !== null ? Carbon::createFromFormat('Y-m-d H:i:s', $temp['created_at'])->copy()->tz($this->response['timezone'])->format('d/m/Y H:i:s') : null;
          $this->response['data'][$i]['end_date'] = $temp !== null ? Carbon::createFromFormat('Y-m-d H:i:s', $temp['updated_at'])->copy()->tz($this->response['timezone'])->format('d/m/Y H:i:s') : null;
